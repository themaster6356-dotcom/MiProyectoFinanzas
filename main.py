...
def main(page: ft.Page):
    ...
    page.bgcolor      = BG
    page.padding      = 0
    page.window_width = 390 # Útil para probar en PC simulando el ancho del celular
    ...
    # Textos de resumen
    txt_balance       = ft.Text("$0.00", size=38, weight=ft.FontWeight.BOLD, color=GREEN, font_family="monospace")
    txt_ing_card      = ft.Text("$0.00", size=15, weight=ft.FontWeight.BOLD, color=GREEN)
    txt_gas_card      = ft.Text("$0.00", size=15, weight=ft.FontWeight.BOLD, color=RED)
    txt_deu_card      = ft.Text("$0.00", size=15, weight=ft.FontWeight.BOLD, color=ORANGE)
    ...
    def seccion_titulo(texto):
        return ft.Text(texto.upper(), size=11, color=MUTED, weight=ft.FontWeight.BOLD, font_family="monospace")
    ...
    def item_lista(icono_emoji, titulo, subtitulo, monto, color_monto, color_bg):
        return ft.Container(
            content=ft.Row([
                ...
                ft.Column([
                    ft.Text(titulo, size=13, color=TEXT, weight=ft.FontWeight.W_500),
                    ft.Text(subtitulo, size=11, color=MUTED),
                ], spacing=2, expand=True),
                ft.Text(monto, size=14, color=color_monto, weight=ft.FontWeight.BOLD, font_family="monospace"),
    ...
    def mostrar_alerta(mensaje, color):
        page.snack_bar = ft.SnackBar(ft.Text(mensaje, color=ft.Colors.WHITE, weight=ft.FontWeight.BOLD), bgcolor=color, duration=2000)
    ...
    def renderizar_listas():
        ...
        todos = [("ingreso", i) for i in datos["ingresos"]] + \
        ...
        for tipo, mov in todos[:6]:
            if tipo == "ingreso":
                lista_recientes.controls.append(item_lista("💹", mov["Concepto"], mov["Fecha"], f"+${float(mov['Monto']):,.2f}", GREEN, "rgba(0,229,160,0.10)"))
            elif tipo == "gasto":
                lista_recientes.controls.append(item_lista("📉", mov.get("Categoría", "Gasto"), mov["Fecha"], f"-${float(mov['Monto']):,.2f}", RED, "rgba(255,92,122,0.10)"))
            else:
                lista_recientes.controls.append(item_lista("👤", mov["Concepto"], mov["Fecha"], f"${float(mov['Monto']):,.2f}", ORANGE, "rgba(255,170,59,0.10)"))

        for i in reversed(datos["ingresos"]):
            lista_ingresos_ui.controls.append(item_lista("💹", i["Concepto"], i["Fecha"], f"+${float(i['Monto']):,.2f}", GREEN, "rgba(0,229,160,0.10)"))

        for g in reversed(datos["gastos"]):
            lista_gastos_ui.controls.append(item_lista("📉", g.get("Categoría", "Gasto"), g["Fecha"], f"-${float(g['Monto']):,.2f}", RED, "rgba(255,92,122,0.10)"))

        for d in reversed(datos["deudas"]):
            lista_deudas_ui.controls.append(item_lista("👤", d["Concepto"], d["Fecha"], f"${float(d['Monto']):,.2f}", ORANGE, "rgba(255,170,59,0.10)"))

        actualizar_resumen()

    # Funciones de lógica de adición (seguras y validan inputs)
    def agregar_ingreso(e):
        try:
            val = float(ingreso_cantidad.value)
            if not ingreso_nombre.value.strip(): raise ValueError
            datos["ingresos"].append({"Fecha": datetime.now().strftime("%Y-%m-%d"), "Concepto": ingreso_nombre.value.strip(), "Monto": val})
            guardar_datos(datos)
    ...
    # Vistas y Contenedores Principales
    ...
    vista_inicio = ft.Column([
        ...
        ft.Container(
            content=ft.Column([
                ft.Text("BALANCE TOTAL", size=11, color="#7aa0cc", font_family="monospace", weight=ft.FontWeight.BOLD),
                txt_balance,
                ft.Text(mes_actual, size=11, color="#5a7a9a"),
            ], spacing=4),
            gradient=ft.LinearGradient(["#1e3a5f", "#0f1e34"], begin=ft.Alignment(-1, -1), end=ft.Alignment(1, 1)),
    ...
    def wrap_vista(controles, visible=False):
        return ft.Column(controls=controles, expand=True, visible=visible, scroll=ft.ScrollMode.AUTO, spacing=12)

    vista_ingreso = wrap_vista([
        ft.Container(height=4),
        ft.Text("Nuevo Ingreso", size=22, weight=ft.FontWeight.BOLD, color=TEXT, font_family="monospace"),
    ...
    vista_gasto = wrap_vista([
        ft.Container(height=4),
        ft.Text("Nuevo Gasto", size=22, weight=ft.FontWeight.BOLD, color=TEXT, font_family="monospace"),
        ft.Text("¿En qué gastaste hoy?", size=12, color=MUTED),
        ft.Container(height=4),
        ft.Text("CATEGORÍA", size=11, color=MUTED, weight=ft.FontWeight.BOLD, font_family="monospace"),
    ...
    vista_deuda = wrap_vista([
        ft.Container(height=4),
        ft.Text("Nueva Deuda", size=22, weight=ft.FontWeight.BOLD, color=TEXT, font_family="monospace"),
    ...
    page.navigation_bar = ft.NavigationBar(
        ...
        destinations=[
            ...
            ft.NavigationBarDestination(icon=ft.Icons.TRENDING_UP_OUTLINED, selected_icon=ft.Icons.TRENDING_UP, label="Ingresos"),
            ft.NavigationBarDestination(icon=ft.Icons.TRENDING_DOWN_OUTLINED, selected_icon=ft.Icons.TRENDING_DOWN, label="Gastos"),
            ft.NavigationBarDestination(icon=ft.Icons.ACCOUNT_BALANCE_WALLET_OUTLINED, selected_icon=ft.Icons.ACCOUNT_BALANCE_WALLET, label="Deudas"),
        ],
